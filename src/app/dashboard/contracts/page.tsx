...
export default function ContractsPage() {
  ...
  useEffect(() => {
    if (currentCompany?.id) fetchContracts();
  }, [currentCompany]);

  const fetchContracts = async () => {
    setLoading(true);
    const res = await fetch(`/api/contracts?company_id=${currentCompany?.id}`);
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link href="/dashboard/contracts/new" className="px-4 py-2 bg-coral-500 text-white rounded-lg hover:bg-coral-600">
          新增合約
        </Link>
      </div>
                ...
                <tr key={c.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="p-4 text-center">
                    <div className="flex justify-center gap-2">
                      ...
                      {c.status === 'draft' && (
                        <button onClick={() => handleGenerateLink(c.id)} className="text-green-600 hover:underline text-sm">產生簽署連結</button>
                      )}
                      {c.status === 'pending_signature' && (
                        <button onClick={() => handleGenerateLink(c.id)} className="text-green-600 hover:underline text-sm">複製連結</button>
                      )}
