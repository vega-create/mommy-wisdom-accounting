...
function calculateNextRunAt(schedule: {
  ...
  if (schedule.schedule_type === 'daily') {
    ...
    const targetMonth = (schedule.schedule_month ?? 1) - 1;
    const targetDate = schedule.schedule_day_of_month ?? 1;
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('*, template:acct_line_templates(id, name)')
...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      company_id, name, recipient_type, recipient_id, recipient_name,
      template_id, custom_content, variables, schedule_type, scheduled_at,
      schedule_time, schedule_day_of_week, schedule_day_of_month,
      schedule_day_of_month_2, schedule_month
    } = body;
    ...
    const next_run_at = calculateNextRunAt({
      schedule_type, scheduled_at, schedule_time, schedule_day_of_week,
      schedule_day_of_month, schedule_day_of_month_2, schedule_month
    });
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (updates.schedule_type || updates.scheduled_at || updates.schedule_time) {
      const next_run_at = calculateNextRunAt({
        schedule_type: updates.schedule_type, scheduled_at: updates.scheduled_at,
        schedule_time: updates.schedule_time, schedule_day_of_week: updates.schedule_day_of_week,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const { error } = await supabase.from('acct_line_schedules').delete().eq('id', id);
    if (error) throw error;
